...
achieves the desired secrecy and privacy properties. Extensions to account for server reaction attacks (see 
Section \ref{sec:security}) are described, though not modelled, as there are multiple ways to build this property
into the protocol.
...
of this to an extension of the core protocol.
...
\end{center}
%
For HelloRetryRequest flows, we also introduce a function $\mathsf{GenerateHRR}$ which 
produces a HelloRetryRequest message $\mathsf{HRR}$ given a ClientHello $\mathsf{CH}$.
...
\resizebox{0.8\columnwidth}{!}{\begin{msc}{}
  ...
  \action*{\laction{Computes:}{
    x \gets \Zp, c \gets \Zp, r \gets \{0,1\}^{32} \\
    Zx = \dh{g^c}{\ksni} \\
    ek, eiv = \mathsf{DeriveESNIKeys}(Zx) \\
    \mathsf{CH} = (g^x, g^c, S, \esni = \seal{ek}{eiv}{g^x}{(r, \sni)})}}{cl}
  \nextlevel[6]
  ...
  \action*{\laction{Computes:}{
    y \gets \Zp \\
    Zx = \dh{\ksni}{g^c} \\
    ek, eiv, r = \mathsf{DeriveESNIKeys}(Zx) \\
    (r', \sni') = \open{ek}{eiv}{g^{x'}}{\esni'}) \\
    ss = \dh{g^y}{g^{x'}} \\
    k' = \kdfpair{ss}{r} \\
...
We developed a model for the core ESNI protocol based on ProVerif \cite{28,32}. ProVerif analyzes symbolic protocol
